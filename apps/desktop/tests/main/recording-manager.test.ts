...
describe("final transcript delivery", () => {
  ...
  it("does not emit a widget notification for no-audio cancellations", () => {
    const manager = createRecordingManager();
    const internals = internalsOf(manager);
    const widgetNotifications: Array<{ type: string }> = [];
    ...
    // Silence is a normal outcome — no toast, the session just cancels.
    expect(widgetNotifications).toEqual([]);
  });

  it("does not emit a widget notification for empty transcripts", async () => {
